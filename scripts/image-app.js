...
function renderImageRatings() {
  ...
  row.innerHTML = IMAGE_RATINGS.map(r => {
    ...
    return `<div class="rating-card ${r.score === 5 ? 'top' : ''}" onclick="filterByRatingCard('ifilter-platform','${r.name}','#page-image .table-section')" style="cursor:pointer" title="点击筛选 ${r.name}">
      <div class="rc-header">
