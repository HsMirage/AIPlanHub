...
function tpGetCommonModelPriority(model) {
  ...
  if (model.startsWith('MiniMax-')) return [7, 1];
  if (model.startsWith('MiMo')) return [8, 0];
  if (model.startsWith('HY-')) return [9, 0];
  if (model.startsWith('Hunyuan')) return [9, 1];
  return [99, 0];
}
...
function renderTpRatings() {
  ...
  row.innerHTML = ratings.map(r => {
    ...
    return `<div class="rating-card ${r.score === 5 ? 'top' : ''}" onclick="tpFilterByRatingCard('${r.name}')" style="cursor:pointer" title="点击筛选 ${r.name}">
      <div class="rc-header">
